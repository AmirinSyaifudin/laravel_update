...
@section('content')
<h1> DATA EDIT FUNGSI pengurangan</h1>
<!-- /.box-header -->
    <div class="box">
        <div class="box-header">
            ...
            <div class="box-body" onload="functionTimer()">
                        <center>
                        <form action="{{ route('admin.pengurangan.update', $pengurangan->pengurangan_id) }}" method="POST" enctype="multipart/form-data" class="form-horizontal" style="max-width:500px">
                            @csrf
                            @method("PUT")
                            <div class="form-group @error('bil_satu') has-error @enderror">
                                <label for="inputPassword3" class="col-sm-2 control-label">BILNGAN SATU</label>
                                    <div class="col-sm-10">
                                        ...
                                        value="{{ $pengurangan->bil_satu ?? old('bil_satu') }}" placeholder="">
                                            @error('bil_satu')
                            ...
                            <div class="form-group @error('bil_dua') has-error @enderror">
                                <label for="inputPassword3" class="col-sm-2 control-label">BILANGAN DUA</label>
                                    <div class="col-sm-10">
                                        ...
                                        value="{{ $pengurangan->bil_dua ?? old('bil_dua') }}" placeholder="Isikan bil_dua Barang">
                                            @error('bil_dua')
                                                <span class="help-block">{{ $message}}</span>
                                            @enderror
                                    </div>
                            ...
                            <div class="form-group">
                                <input type="hidden" name="pengurangan_id" value="{{$pengurangan->pengurangan_id}}">
                                <input type="submit" value="Update" class ="btn btn-primary">
                                <button type="reset" class="btn btn-warning">Ulang</button>
                                <a href="{{ route('admin.pengurangan.index') }}" class="btn btn-danger">Kembali</a>
                            </div>
                        </form>
                        </center>
        </div>
