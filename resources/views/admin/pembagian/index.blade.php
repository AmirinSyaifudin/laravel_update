    ...
    <div class="box">
        <div class="box-header">
            ...
            <table id="dataTable" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        @foreach ($pembagian as $gatel)
                        <tr>
                            <td width='5'>  {{ $loop-> index +1 }} </td>
                            <td width='20'> {{ $gatel->bil_satu }}</td>
                            <td width='20'> {{ $gatel->bil_dua }}</td>
                            <td width='5'>  {{ $gatel->jumlah }}  </td>
                            <td width='5'>   </td>
                            <td width='5'>  <a href="{{ route('admin.pembagian.edit', $gatel->pembagian_id) }}" class="btn btn-warning">EDIT</a></td>
                            <td width='5'>
                                <form action="{{ route('admin.pembagian.destroy', $gatel->pembagian_id) }}" method="post" style="display:inline;">
                                    {{ csrf_field() }}
                                    {{ method_field ('delete')}}
