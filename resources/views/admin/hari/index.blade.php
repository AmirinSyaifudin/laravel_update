    ...
    <div class="box">
        <div class="box-header">
            <center> <h3 class="box-title" style="text-transform: uppercase;">Silahkan input data </h3><br><br> </center>
                <form action="{{ route('admin.hari.store') }}" class="form-horizontal" method="POST" enctype="multipart/form-data" style="max-width:500px">
                    @csrf
                    ...
                    <div class="form-group @error('tanggal_lahir') has-error @enderror">
                        <label for="inputPassword3" class="col-sm-2 control-label">tanggal_lahir</label>
                            <div class="col-sm-10">
                    ...
                    <br>
                    Tanggal Lahir : 
                    <input type="text" name="texttg" size="2" maxlength="2">
                    <input type="text" name="textbl" size="2" maxlength="2">
                    <input type="text" name="textth" size="4" maxlength="4">
            ...
            <table id="dataTable" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        @foreach ($hari as $gatel)
                        <tr>
                            <td width='5'>  {{ $loop-> index +1 }} </td>
                            <td width='20'> {{ $gatel->nama }} </td>
                            <td width='20'>  {{ tanggal_indonesia($gatel->tanggal_lahir)}} </td>
                            <td width='20'><a class="btn btn-block btn-primary btn-sm"> {{ $gatel->keterangan }}</a></td> 
                            ...
                            <td width='5'>
                                <form action="{{ route('admin.hari.destroy', $gatel->hari_id) }}" method="post" style="display:inline;">
                                    {{ csrf_field() }}
                                    {{ method_field ('delete')}}
